...
class PayrollPeriodSelectController extends Controller
{
    public function get()
    {
        // 期間の選択が可能な最初の月
        ...
        return view ('attendance.payroll_period_select',['data'=>$data]);        
    }
    ...
    public function post(Request $request)
    {
        ...
        // セッションへ$to,$fromを保存
        if(!empty($request)){
            session()->put('to_payroll', $to);
            session()->put('from_payroll', $from);
        }

        return redirect ('/payroll/display');
    }
}

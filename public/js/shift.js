...
function createProcess(year, month) {
    // 曜日
    var calendar = "<table class=\"table table-bordered table-responsive\"><tr class='dayOfWeek'>";
    ...
    for (var i = 0; i < row; i++) {
        ...
        for (var j = 0; j < week.length; j++) {
            if (i == 0 && j < startDayOfWeek) {
                ...
                // 最終行で最終日以降、翌月の日付を設定
                count++;
                calendar += "<th class='disabled'>" + (count - endDate) + "</th>";
            } else {
                // 当月の日付を曜日に照らし合わせて設定
                count++;
                if(year == today.getFullYear()
                  ...
                  && count == today.getDate()){
                    calendar += "<th class='today'>" + count + "</th>";
                } else {
                    calendar += "<th>" + count + "</th>";
                }
            }
        }
        calendar += "</tr>";
